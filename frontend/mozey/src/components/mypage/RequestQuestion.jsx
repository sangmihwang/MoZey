...
const RequestQuestion = () => {
  ...
  const getP = async () => {
    ...
    try {
      ...
      await axios
        .get(
          `https://j9a510.p.ssafy.io/api/questions/users/${id}/P/?pageIdx=${pageIdxP}`
        )
        .then((data) => {
  ...
  const getY = async () => {
    ...
    try {
      ...
      await axios
        .get(
          `https://j9a510.p.ssafy.io/api/questions/users/${id}/Y/?pageIdx=${pageIdxY}`
        )
        .then((data) => {
          if (data.data.message === "질문 신청 내역 조회 - 사용자") {
  ...
  const deleteQtn = (qtnId) => {
    try {
      axios
        .delete(`https://j9a510.p.ssafy.io/api/questions/${qtnId}`)
        .then((data) => {

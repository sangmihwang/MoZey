...
const MessageInfo = ({ dataforMessageInfo, selectedInfo }) => {
  ...
  useEffect(() => {
    ...
    if (isSub === 0 && dataforMessageInfo.userId) {
      axios
        .get(
          `https://j9a510.p.ssafy.io/api/users/extract/${dataforMessageInfo.userId}`
        )
        .then((response) => {
      ...
      if (selectedInfo.type === "class") {
        axios
          .get(
            `https://j9a510.p.ssafy.io/api/users/extract/${dataforMessageInfo.userId}/select/${selectedInfo.type}`
          )
          .then((response) => {
            setSecretUserData(response.data);
          })
        ...
        axios
          .get(
            `https://j9a510.p.ssafy.io/api/users/extract/${dataforMessageInfo.userId}/location/${selectedInfo.value}`
          )
          .then((response) => {
      ...
      <S.FindoutBox>
        ...
        {isSub === 1 && selectedInfo.type === "class" && (
          <S.ClassContainer>
            ...
            <S.ClassBox>{secretUserData.data?.value}</S.ClassBox>
          </S.ClassContainer>
        )}
        <S.FriendName>
          <S.NameBox>
            {selectedInfo.type === "location" &&
            secretUserData.data?.location === 1
              ? secretUserData.data?.consonant
              : "X"}
          </S.NameBox>
          <S.NameBox>
            {selectedInfo.type === "location" &&
            secretUserData.data?.location === 2
              ? secretUserData.data?.consonant
              : "X"}
          </S.NameBox>
          <S.NameBox>
            {selectedInfo.type === "location" &&
            secretUserData.data?.location === 3
              ? secretUserData.data?.consonant
              : "X"}
          </S.NameBox>
        </S.FriendName>
